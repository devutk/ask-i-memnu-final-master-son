...
public class LevelManager : MonoBehaviour
{
    ...
    [Header("Parfum Bar UI (parfumbar hierarchy)")]
    [SerializeField] TextMeshProUGUI perfumeCountText;
    [SerializeField] GameObject perfumeReadyWarning;
    ...
    void Start()
    {
        ...
        bihterZone = FindAnyObjectByType<bihterzone>();
        npcVisions = FindObjectsByType<npcvision>(FindObjectsInactive.Exclude);

        GameObject[] perfumes = GameObject.FindGameObjectsWithTag("Perfume");
    ...
    void Gameplay()
    {
        ...
        // 3. FILL PROGRESS (tum parfumler toplanmis ve Bihter'in yaninda kaldikca bar dolar)
    ...
    public void CollectPerfume()
    {
        ...
        // Tum parfumler toplandiginda bir kez 3 saniyelik uyari goster
        if (!readyShown && HasAllPerfumes())
        {
            readyShown = true;
            ShowReadyWarning();
        }
    ...
    void ShowReadyWarning()
    {
        if (perfumeReadyWarning == null) return;
        StopAllCoroutines();
    ...
    {
        perfumeReadyWarning.SetActive(true);
        yield return new WaitForSeconds(warningDuration);
        if (perfumeReadyWarning != null) perfumeReadyWarning.SetActive(false);
    }
    ...
    void SetupParfumBar()
    {
        ...
        BindParfumBarUI(bar.transform);
    }

    void BindParfumBarUI(Transform bar)
    {
        if (perfumeCountText == null)
            perfumeCountText = bar.Find("ParfumBarCanvas/PerfumeCount")?.GetComponent<TextMeshProUGUI>();

        if (perfumeReadyWarning == null)
            perfumeReadyWarning = bar.Find("ParfumBarCanvas/PerfumeReadyWarning")?.gameObject;

        if (perfumeReadyWarning != null)
            perfumeReadyWarning.SetActive(false);

        UpdatePerfumeCount();
    }

    void UpdatePerfumeCount()
    {
        if (perfumeCountText != null)
            perfumeCountText.text = collectedPerfumes + " / " + totalPerfumes;
    }
